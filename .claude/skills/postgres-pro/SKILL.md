...
El destino es **Neon** (Postgres serverless), no un Postgres autogestionado. Buena parte del
apartado operativo de esta skill **no aplica**:

| No aplica en Neon | Motivo |
|---|---|
| Tuning de `postgresql.conf`, `shared_buffers`, `work_mem` | lo gestiona Neon |
| Configuración de réplicas y failover | lo gestiona Neon |
| `VACUUM`/autovacuum tuning manual | gestionado; útil solo para diagnosticar |
| `pg_stat_statements` autoinstalado | disponible según plan; se activa desde el panel |

**Sí aplica, y mucho:** `EXPLAIN (ANALYZE, BUFFERS)`, diseño y verificación de índices,
operadores de `pg_trgm` (`%`, `similarity()`, `set_limit()`), `unaccent`, JSONB
(`import_job.report`, `ai_job.result`), y evitar N+1.

Particularidades de Neon que mandan sobre lo que diga la skill:

- **Dos cadenas de conexión.** La *pooled* para la app; la **unpooled**
  (`DATABASE_URL_UNPOOLED`) para migraciones, seed y scripts. El DDL largo no va por el pooler.
- **Driver HTTP** (`neon(...)`) en Route Handlers y Server Actions: no mantiene socket, que es
  lo correcto en serverless. `Pool` (WebSocket) solo en scripts CLI, donde sí hay
  transacciones largas.
- **Cold start**: la primera query tras inactividad tarda. No es un problema de índices; no
  persigas ese fantasma en un `EXPLAIN`.
- **Branches de Neon**: son la red de seguridad para probar una migración de riesgo contra
  una copia de producción antes de aplicarla de verdad. Úsalos.
