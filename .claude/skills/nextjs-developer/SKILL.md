...
Versión objetivo: **Next.js 15 con App Router**, TypeScript estricto, despliegue en Vercel.
Decisiones ya tomadas en este repo:

- Mutaciones: **Server Actions** para lo que nace de la UI; **Route Handlers** para binarios,
  subidas, descargas y procesos largos. Ver `.claude/rules/api-conventions.md`.
- `middleware.ts` protege **todo** `/app/*` con Auth.js v5; sin sesión → `/login`.
  La landing `/` es pública.
- Cabeceras de seguridad (CSP con nonce, `nosniff`, `Referrer-Policy`, HSTS) en
  `next.config.ts`. Ver `.claude/rules/security.md` §6.
- Fuentes con `next/font/google` (Cormorant Garamond 300/400, Inter 400/500/600,
  IBM Plex Mono 400) — y ningún peso más.
- **Nada dependiente de sesión se cachea.** La revalidación por etiqueta **siempre** lleva el
  `userId` dentro (`revalidateTag("animes:" + userId)`): una etiqueta global serviría la caché
  de un usuario a otro.
- Única excepción de caché agresiva: `GET /api/covers/[animeId]` con
  `Cache-Control: public, max-age=31536000, immutable` + `ETag` = checksum. Es seguro porque se
  comprueba la propiedad antes de servir y la URL va versionada por checksum.
- **`runtime = "nodejs"`** en todo lo que use `sharp`, Argon2 o el driver de Neon con `Pool`.
  El runtime edge no vale para eso.
