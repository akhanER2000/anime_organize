...
La documentación de este repo va **en español**, como el dominio y la interfaz.

Piezas y su dueño:

| Archivo | Contenido |
|---|---|
| `README.md` | crear la BD en Neon, conectar Vercel, variables, migraciones, seed, desplegar |
| `CLAUDE.md` | qué es el proyecto, stack, comandos, arquitectura, enlaces a `rules/` |
| `.claude/rules/*.md` | las normas por área; son **normativas**, no descriptivas |
| `.claude/skills/anime-vault-domain/` | las reglas de dominio que no deben reinventarse |
| `CHANGELOG.md` | lo implementado por fase y **lo que quedó fuera** |
| `.env.example` | todas las variables, cada una con **dónde se obtiene** |

No se documenta lo que el código ya dice. Se documenta **el porqué**: por qué la URL de una
portada no es el almacenamiento, por qué el umbral de similitud es 0.55, por qué
`White Album 2` no puede colapsar con `White Album`.
