...
React 19 sobre Next.js 15 (App Router). Precisiones del repo:

- **Server Component por defecto**; `"use client"` es la excepción justificada y va lo más abajo
  posible. Un layout no se vuelve cliente porque un botón necesite `onClick`: se extrae el botón.
- Formularios: **react-hook-form + `zodResolver`**, con el **mismo** esquema Zod que usa el
  servidor (`src/lib/validation/`). El cliente valida por UX; el servidor valida por seguridad
  y nunca se fía del cliente.
- **UI optimista** obligatoria al marcar progreso (`useOptimistic`): +1 episodio se pinta al
  instante y se revierte si el servidor falla.
- Nada de librerías de estado global (Redux, Zustand, Jotai). El estado de filtros vive en la
  **URL** (`searchParams`); el resto es estado local o del servidor.
- Nada de `useEffect` + `fetch` para la primera pintura: los datos se cargan en el servidor.
- El debounce del buscador es de **250 ms** y está fijado por el enunciado.
