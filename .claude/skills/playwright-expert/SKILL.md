...
Vale tal cual: el proyecto usa `@playwright/test` en TypeScript. Precisiones del repo:

- El e2e vive en `e2e/`, corre contra `npm run build && npm run start` (nunca `dev`) y solo cubre el
  **flujo crítico** definido en `.claude/rules/testing.md`: registro → añadir anime con URL de
  imagen → **verificar que la portada se sirve desde `/api/covers` y no desde el dominio
  original** → filtrar → eliminar cuenta.
- Nada de Page Objects para una suite de cuatro specs: es abstracción prematura. Helpers
  simples en `e2e/helpers/`.
- La verificación de la portada se hace **interceptando la red** (`page.route` /
  `page.on("request")`), no solo mirando el `src`.
- Sin snapshots visuales: la fidelidad al diseño la comprueba el subagente
  `ui-fidelity-checker` contra `design/screens/`, no un diff de píxeles frágil.
