...
Esta skill asume **Python** (`openpyxl`, `pandas`, `markitdown`, `scripts/recalc.py` con
LibreOffice). **Anime Vault no tiene Python en el runtime**: es Next.js sobre Node en Vercel.

**Traducción obligatoria al stack del proyecto:**

| La skill dice | Aquí se hace con |
|---|---|
| `openpyxl` / `pandas` | **SheetJS** (`xlsx` de npm), en TypeScript |
| `load_workbook(path)` | `XLSX.read(buffer, { type: "buffer", cellDates: true })` |
| `pd.read_excel` | `XLSX.utils.sheet_to_json(hoja, { defval: null, raw: false })` |
| `wb.save(path)` | `XLSX.write(wb, { type: "buffer", bookType: "xlsx" })` |
| `scripts/recalc.py` | **no aplica**: no generamos fórmulas |

**Lo que sí se conserva de la skill** (es agnóstico del lenguaje y sigue siendo válido):
seguir la especificación del usuario al pie de la letra, respetar los nombres de pestaña y
de cabecera, documentar los supuestos, y no redondear ni reinterpretar datos ajenos.

**Lo que NO aplica aquí:** «usa fórmulas, nunca resultados fijos» y el paso de recálculo.
Nuestros dos usos son de datos planos, sin fórmulas:

1. **Importación** (Ajustes → Importar): se lee `.xlsx`/`.csv` **subido por el usuario**, o sea
   entrada no confiable. Se valida por magic bytes y tamaño **antes** de parsear, se limita el
   número de filas, y cada fila pasa por Zod. Una hoja con 200 000 filas no puede tumbar la
   función serverless.
2. **Exportación**: se genera el `.xlsx` del vault en memoria y se devuelve como descarga
   desde un Route Handler, sin escribir a disco (en Vercel el sistema de ficheros es efímero).

**Nunca** se ejecuta una fórmula, una macro ni un `=HYPERLINK` que venga del fichero subido.
Las celdas se tratan como texto.
