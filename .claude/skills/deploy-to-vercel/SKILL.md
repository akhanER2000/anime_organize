...
El despliegue de este proyecto pasa **siempre** por `/project:deploy`, que añade lo que esta
skill no sabe:

1. Puertas de calidad **antes** de subir: `typecheck`, `lint`, `lint:tokens`, `test`, `build`
   (y `test:e2e` para producción). No se despliega «a ver si en Vercel va».
2. **Escaneo de secretos** en el diff antes de subir.
3. **Migraciones antes del despliegue**, con `DATABASE_URL_UNPOOLED`.
   `db:push` **jamás** contra producción.
4. Verificación posterior: `/` pública, `/app` redirige sin sesión, las portadas salen de
   `/api/covers`, y las cabeceras de seguridad están presentes.

Variables imprescindibles en Vercel: `DATABASE_URL`, `DATABASE_URL_UNPOOLED`, `AUTH_SECRET`
(distinto en preview y producción), `AUTH_URL`. Opcionales: `ANTHROPIC_API_KEY`,
`AUTH_GOOGLE_*`, `GOOGLE_DRIVE_*`, `EMAIL_*`. Ver `.env.example`.

Detalle de runtime: las rutas con `sharp`, Argon2 o `Pool` de Neon necesitan
**`runtime = "nodejs"`**. No funcionan en edge.
