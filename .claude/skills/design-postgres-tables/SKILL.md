...
La skill escribe **DDL SQL a mano**. Aquí el DDL es **generado**: la fuente de verdad es el
esquema de **Drizzle** en `src/lib/db/schema/*.ts`, y el SQL sale de `npm run db:generate`.
Se lee y se revisa el SQL, pero no se escribe a mano salvo para corregir una migración.

Decisiones ya tomadas en este proyecto (**no las vuelvas a debatir**, están en
`.claude/rules/db-conventions.md`):

- **`uuid` para toda entidad de usuario**, no `identity`/`serial`: un id secuencial filtra
  volumen y permite enumerar.
- **«Enums» como `text` + `CHECK`**, no `pgEnum`: añadir un valor a un enum de Postgres es una
  migración incómoda; un `CHECK` se reemplaza sin drama.
- **`timestamptz` siempre**, nunca `timestamp` sin zona.
- **`citext`** para email, con `UNIQUE`.
- **`numeric(3,1)`** para la puntuación, nunca `float`.
- **`bytea`** para las portadas: los binarios viven en la BD, que es la fuente de verdad.
  Regla asociada: `bytes`/`thumb_bytes` **jamás** se seleccionan en un listado.
- **Sin Row Level Security.** El aislamiento entre usuarios se garantiza en la capa de
  repositorio (`requireOwnedAnime`) y se verifica con tests de integración con dos usuarios.
  No usamos Supabase ni sus políticas RLS.
- Índice **GIN con `pg_trgm`** sobre `title_normalized`: es el que sostiene la deduplicación
  difusa (umbral 0.55) y el buscador. Ver `anime-vault-domain`.
