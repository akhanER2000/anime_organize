...
Aplica entera en la parte de ingeniería (accesibilidad, responsive, estado, calidad de
implementación). Precisiones del proyecto:

- **La apariencia no se decide aquí**: sale de `design/`. Ver la adaptación de `frontend-design`
  y `.claude/rules/design-tokens.md`. Cero hex fuera de `globals.css`.
- **Tailwind v4 con tokens `@theme`.** Nada de CSS-in-JS, `styled-components` ni una librería
  de componentes de terceros (shadcn, MUI, Chakra): el sistema de diseño es propio y está en
  `src/components/ui/`.
- **Server Component por defecto.** `"use client"` lo más abajo posible del árbol, solo cuando
  hay estado, evento o API del navegador.
- Accesibilidad, no negociable y ya especificada: foco visible siempre (anillo 2 px
  `--gold-400`, offset 2 px), contraste ≥4.5:1, área táctil ≥44 px en móvil, navegación
  completa por teclado (`/` enfoca el buscador, `Esc` limpia), y **el estado nunca se comunica
  solo por color**: cada badge lleva etiqueta de texto.
- **`prefers-reduced-motion`** ya está resuelto en `tokens.css`: desactiva shimmer, spinner y
  transiciones. No lo reimplementes.
- Los ocho estados de cada componente (default, hover, focus, active, disabled, loading, error,
  vacío) están tabulados en `design/DESIGN-SPEC.md` §6. No se inventan.
