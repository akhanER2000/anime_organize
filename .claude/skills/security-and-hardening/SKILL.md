...
Aplica. Su contenido está concretado para este repo en **`.claude/rules/security.md`**, que
manda en caso de conflicto.

Lo específico de Anime Vault que la skill genérica no puede saber:

- El endpoint que descarga URLs del usuario (`/api/covers`) es la superficie crítica: SSRF.
- La app es **multiusuario sin RLS**: el aislamiento lo garantiza la capa de repositorio y se
  verifica con tests de integración con dos usuarios sembrados.
- **En serverless, un rate limiter solo en memoria no funciona**: cada instancia tiene el suyo.
  El backing store debe ser Postgres o Upstash en producción.
- La CSP permite exactamente tres orígenes externos: `graphql.anilist.co`,
  `api.anthropic.com` y `fonts.gstatic.com`. Añadir un cuarto es una decisión consciente.
- `sharp` re-encodea toda imagen entrante a WebP: además de normalizar, destruye payloads
  (SVG con script, polyglots, EXIF con GPS). Es una medida de seguridad, no solo de formato.
