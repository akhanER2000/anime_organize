...
Aplica entera. En este proyecto sus reglas ya están concretadas en
**`.claude/rules/security.md`**, que **manda** sobre cualquier ejemplo genérico de la skill.

Las cuatro superficies de esta app, por orden de peligro:

1. **SSRF en `POST /api/covers`** — descarga una URL que controla el usuario. Es el punto más
   peligroso del repo. La lista completa de bypasses (IPs privadas, metadata de nube
   `169.254.169.254`, decimal/octal, IPv4 mapeada en IPv6, redirección 302 a privada, **DNS
   rebinding**, credenciales embebidas, magic bytes) está en `security.md` §4 y tiene un test
   por caso.
2. **Aislamiento por `user_id`** — ninguna query cruza usuarios. Un recurso ajeno responde
   **404**, nunca 403.
3. **Borrado de cuenta** — rate limit → re-autenticación → escribir el email exacto → export
   `.json` **antes** de borrar → cascada real en transacción.
4. **Prompt injection** en el enriquecimiento — la sinopsis es dato, no instrucción, y la
   respuesta se valida con Zod cerrado o se descarta.

Hash de contraseñas: **Argon2id** (`@node-rs/argon2`, `m=19456 t=2 p=1`); bcrypt ≥12 como
fallback documentado. Nunca SHA/MD5 pelado.

**Nota de stack:** los ejemplos de la skill que asuman Express, Passport o middlewares de
Node clásico se traducen a **Auth.js v5 + Route Handlers/Server Actions de Next.js**.
Aquí no hay Express.
