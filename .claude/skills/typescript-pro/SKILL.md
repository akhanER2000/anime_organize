...
Aplica, con el rigor que fija `.claude/rules/code-style.md`:

- `strict` + `noUncheckedIndexedAccess` + `exactOptionalPropertyTypes` + `verbatimModuleSyntax`.
- **`any` prohibido** (falla el lint). **`as unknown as X` prohibido.**
  **`!` de non-null prohibido.**
- Los datos que cruzan una frontera (HTTP, BD, fichero, IA) se **parsean con Zod**, no se
  castean. `EsquemaX.parse(json)` da un tipo real; `json as X` es una mentira tipada.
- Tipos derivados de **una** fuente: los de BD desde Drizzle (`typeof anime.$inferSelect`),
  los de API desde Zod (`z.infer<typeof EsquemaX>`). Nunca reescritos a mano en dos sitios.

**No aplica:** el apartado de **tRPC**. Este proyecto usa Server Actions + Route Handlers con
el sobre `{ ok, data | error }`; añadir tRPC sería una tercera vía de mutación, y la regla es
que solo hay dos. Los tipos ya viajan de extremo a extremo vía Zod.

Tampoco aplica la configuración de monorepo: esto es un único paquete.
