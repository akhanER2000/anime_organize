...
Skill de **post-despliegue**: necesita métricas reales de un proyecto ya desplegado
(invocaciones, Fast Data Transfer, minutos de build, Core Web Vitals). **No la uses durante el
desarrollo inicial**: sin datos, sus recomendaciones son adivinanzas.

Cuándo sí, en este proyecto: después de la FASE 6, con el vault real desplegado y unos días de
tráfico.

Puntos de coste previsibles que conviene mirar cuando llegue el momento:

- **`GET /api/covers`** es la ruta con más invocaciones (una por portada por usuario nuevo).
  Ya lleva `immutable` + ETag + 304, que es exactamente la mitigación correcta.
- **`sharp`** en Node runtime infla el bundle de la función y el tiempo de arranque en frío.
- **`/api/enrich/batch`** es la ruta con más duración: por eso está diseñada como cola con
  polling y no como una petición larga (en Vercel se cortaría por timeout).
- `design/` ya está excluido del despliegue en `.vercelignore` (85 MB de PNG que no se sirven).
