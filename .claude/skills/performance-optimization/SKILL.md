...
Aplica, con dos precisiones del proyecto:

- **Medir antes de tocar.** El vault son 83 filas: casi ninguna optimización teórica se nota.
  Optimizar sin un `EXPLAIN` o un perfil es deuda disfrazada de virtud.
- Los dos riesgos **reales** aquí, y son de diseño, no de micro-optimización:
  1. **N+1** al pintar la biblioteca (83 cards × progreso × portada × géneros).
  2. **Traer binarios donde no tocan**: `anime_cover.bytes` en un listado son megabytes por
     petición. Solo se leen en `/api/covers/[animeId]`.
- La búsqueda difusa debe usar el índice **GIN trigram**. Si el `EXPLAIN` muestra un *seq scan*
  sobre `anime`, es porque la query aplica una función encima de la columna: eso invalida el
  índice. Se normaliza en la app, no en el `WHERE`.
- Cold start de Neon: la primera query tras inactividad tarda y **no** es un problema de
  índices. No persigas ese fantasma.
