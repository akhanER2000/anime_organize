...
La skill instruye usar `claude-opus-5` por defecto. **En este proyecto el usuario eligió
explícitamente la gama Sonnet** («modelo claude-sonnet-4-5 o superior»). Como
`claude-sonnet-4-5` ya no existe en la tabla vigente, el equivalente actual o superior de esa
gama es **`claude-sonnet-5`**, y ese es el valor por defecto de `ANTHROPIC_MODEL`.

Se puede cambiar por variable de entorno sin tocar código. **No lo cambies por tu cuenta**:
es una decisión del usuario.

Uso concreto en Anime Vault (paso 2 del enriquecimiento, `src/lib/enrich/claude.ts`):

- Tarea de **clasificación con salida estructurada**: etiquetas de un vocabulario cerrado de
  26 slugs, tono, público, advertencias y un resumen ≤200 caracteres en español.
- La respuesta se valida con **Zod estricto**. Si no valida, **se descarta entera** y se
  registra el error en `ai_job`. Nunca se guarda «lo que haya devuelto».
- La sinopsis viene de AniList o del usuario: es **dato, no instrucción**. El prompt lo declara
  explícitamente. Prompt injection es un riesgo real en este flujo.
- Se registran `tokens_in` / `tokens_out` por trabajo en `ai_job`.
- **Sin `ANTHROPIC_API_KEY` el paso 2 se salta con un aviso** (`IA_NO_CONFIGURADA`) y el paso 1
  (AniList) sigue funcionando. Eso es comportamiento correcto, no un fallo.
- Prompt caching: el bloque de sistema con el vocabulario controlado es idéntico en las 83
  llamadas del lote. Es el candidato natural a `cache_control`.
