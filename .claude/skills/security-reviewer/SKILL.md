...
Para auditar **este** repo, usa el subagente **`security-auditor`**
(`.claude/agents/security-auditor.md`): ya lleva la lista concreta de ataques de esta app
(SSRF de `/api/covers`, aislamiento por `user_id`, borrado de cuenta, rate limits) en vez de
un checklist genérico.

Esta skill aporta el **formato de informe** (severidad, ubicación, impacto, remediación) y la
metodología de auditoría, que sí se usan.

**No aplica en este proyecto:** el apartado de infraestructura y cloud (IAM, VPC, buckets,
Kubernetes). Aquí solo hay Vercel + Neon gestionados; no hay infraestructura que auditar.

Es **obligatorio** pasar una auditoría antes de cerrar cualquier cambio que toque
`src/lib/covers/`, el flujo de auth o el borrado de cuenta.
