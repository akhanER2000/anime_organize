...
Se usa para los diagramas del `README.md` y de la documentación de arquitectura. La skill
asume contexto científico/académico; aquí el contexto es documentación de producto.

Diagramas que este proyecto sí quiere:

- **ER** del modelo de datos (`users → anime → cover/progress/genre/link`).
- **Secuencia** del pipeline de portadas (URL → validación SSRF → sha256 → sharp → BD → servido
  con ETag).
- **Secuencia** del enriquecimiento en dos pasos (AniList → Claude → validación Zod).
- **Flujo** de deduplicación (exacto → `anilist_id` → similitud 0.55 → decisión del usuario).

Texto **en español**. Los diagramas son para explicar decisiones no evidentes, no para
adornar: un diagrama que repite lo que ya dice una tabla sobra.
