...
El proyecto ya tiene su contrato cerrado en `.claude/rules/api-conventions.md`. **Manda esa
regla**, no las alternativas que plantee la skill. En concreto, no se vuelve a discutir:

- **No hay GraphQL.** Solo Server Actions (formularios y botones de la UI) y Route Handlers
  (binarios, subidas, descargas, procesos largos). Ninguna tercera vía.
  (AniList sí es GraphQL, pero es un proveedor **externo** que consumimos.)
- **No hay versionado en la URL** (`/api/v1/...`). Es una app personal de un solo cliente:
  el versionado sería ceremonia sin beneficio.
- **Sobre de respuesta fijo:** `{ ok: true, data }` | `{ ok: false, error: { codigo, mensaje, detalles? } }`.
  El cliente ramifica por `codigo`, nunca por `mensaje`. Los mensajes van **en español**.
- **Paginación por keyset** (`cursor`), no `OFFSET`.
- **Sin OpenAPI generado.** Los esquemas Zod de `src/lib/validation/` son el contrato, y son
  la misma pieza que valida en runtime: un spec aparte se desincronizaría.
- Rutas en **español**, `kebab-case`, sustantivo plural, sin verbos (el verbo es el método).
  Excepciones fijadas por el enunciado: `/api/covers` y `/api/enrich`.
